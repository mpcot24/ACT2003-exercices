...
\begin{exercice}
  ...
  \begin{sol}
    ...
    \texttt{horsepower}:
<<echo=TRUE>>=
autoprice <- read.table("auto-price.dat", header = TRUE)
...
\end{Soutput}
  ...
  \begin{sol}
    ...
    \item On doit se fier ici au résultat du test $t$ associé à la
      ...
      faveur de $H_1: \beta_2 \neq 0$. Il s'agit néanmoins d'un cas
...
\begin{exercice}
  ...
  \end{center}
<<echo=FALSE>>=
donnees <- read.table("exercice_3.16.dat", header = TRUE)
...
\begin{exercice}
  ...
  \end{enumerate}
<<echo=FALSE>>=
donnees <- read.table("exercice_3.17.dat", header = TRUE)
...
\begin{exercice}
  ...
  \end{enumerate}
<<echo=FALSE>>=
donnees <- read.table("exercice_3.18.dat", header = TRUE)
