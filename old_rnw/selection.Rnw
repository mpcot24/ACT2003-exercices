...
\begin{exercice}
\filename{ethnicite_compagnies_assurance}
\title{Utilisation de l'ethnicité par les compagnies d'assurance}
...
\begin{exercice}
\filename{matrice_incidence_diagonale}
\title{Matrice d'incidence diagonale}
...
\begin{exercice}
\filename{ridge_main}
\title{Régression Ridge à la main}
...
\begin{exercice}
\filename{ridge_biais}
\title{Biais de l'estimateur du modèle Ridge}
