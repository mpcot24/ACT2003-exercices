...
\begin{exercice}
\filename{10points_reglinsimple}
\title{Petit jeu de données pour une régression linéaire simple}
  ...
  \end{center}
  \begin{enumerate}
  \item Placer les points ci-dessus sur un graphique.
  \item Calculer les équations de score. \label{ex:simple:base:eq_normales}
  ...
  \item Calculer les prévisions $\hat{Y}_i$ correspondant à $x_i$ pour
    ...
    fait en a).
  \item Vérifier empiriquement que $\sum_{i=1}^{10} \hat{\varepsilon}_i = 0$.
  \end{enumerate}
...
@
      ...
      \end{figure}
    \item Les résidus de la régression sont $\hat{\varepsilon}_i = Y_i - \hat{Y}_i$,
...
\begin{exercice}
\filename{estimateurs_moindres_carres}
\title{Estimateurs des moindres carrés pour la régression linéaire simple}
...
\begin{exercice}
\filename{estimateurs_moindres_carres_origine}
\title{Estimateurs des moindres carrés pour la régression linéaire passant par l'origine}
  ...
  \item Trouver l'estimateur des moindres carrés du paramètre $\beta$
    ...
    \beta x_i + \varepsilon_i$, $i = 1, \dots, n$, sous les postulats
    $\esp{\varepsilon_i} = 0$, et $\cov{\varepsilon_i, \varepsilon_j} =
    \mathbf{1}(i=j) \sigma^2$.
...
\begin{exercice}
\filename{sexe_esperance_vie}
\title{Effet du sexe sur l'espérance de vie}
...
\begin{exercice}
\filename{emc_blue}
\title{Démonstration que l'EMC est le BLUE}
  ...
  \end{displaymath}
  on obtient $\beta^* = \hat{\beta}$. \emph{Indice: le lagrangien permet de faire l'optimisation sous contrainte, voir l'annexe~D des notes de cours.}
  \begin{sol}
...
\begin{exercice}
\filename{ic_beta1}
\title{Intervalle de confiance pour $\beta_1$}
...
\begin{exercice}
\filename{prix_gaz_naturel}
\title{Relation entre la consommation de gaz naturel et le prix}
  ...
  \begin{align*}
    ...
    \sum_{i=1}^{20} x_i^2 &= \nombre{90048} &
      \sum_{i=1}^{20} \hat{\varepsilon}_i^2 &= \nombre{7832}. \\
    \sum_{i=1}^{20} Y_i^2 &= \nombre{116058}
  \end{align*}
...
\begin{exercice}
\filename{ic_reglinsimple_origine}
\title{Intervalle de confiance pour la régression linéaire passant par l'origine}
...
\begin{exercice}
\filename{massemonetaire_pib}
\title{Relation entre la masse monétaire et le PIB}
...
\begin{exercice}
\filename{demonstrations_mse_msr}
\title{Démonstrations avec le MSE et le MSR}
  ...
  \begin{sol}
    ...
    \item Tout d'abord, puisque $\MSE = \SSE/(n - 2) = \sum_{i=1}^n
      ...
      \begin{align*}
        ...
        &= \frac{\sigma^2}{n} + \frac{(x_i -
          \bar{x})^2 \sigma^2}{\sum_{j=1}^n (x_j - \bar{x})^2}.
      \end{align*}
      Par conséquent,
      \begin{align*}
        \esp{(Y_i - \hat{Y}_i)^2}
    ...
    \item On a
      \begin{align*}
        ...
        &= \sum_{i=1}^n (x_i - \bar{x})^2
        \left(
          \frac{\sigma^2}{\sum_{j=1}^n (x_j - \bar{x})^2} + \beta_1^2
        \right) \\
...
\begin{exercice}
\filename{effet_temperature_procede_chimique}
\title{Effet de la température sur un procédé chimique}
...
\begin{exercice}
\filename{ligne_de_vie}
\title{Relation entre l'espérance de vie et la longueur de la ligne de vie}
...
\title{Démonstration}
  ...
  \begin{sol}
    ...
    que $\hat{\varepsilon}_i = Y_i - \hat{Y}_i = (Y_i - \bar{Y}) - \hat{\beta}_1 (x_i
    - \bar{x})$, alors
    \begin{align*}
...
\begin{exercice}
\filename{covariance_residus}
\title{Covariance entre deux résidus}
...
&=0-2\sigma^2\left(\frac{1}{n}+\frac{(x_j-\bar{x})(x_i-\bar{x})}{S_{xx}}\right)+\sigma^2\left(\frac{1}{n}+\frac{(x_j-\bar{x})(x_i-\bar{x})}{S_{xx}}\right)\\
...
\title{Analyse de variance avec un tableau de données}
  ...
  \item Calculer les coefficients de la régression $Y_i = \beta_0 +
    \beta_1 x_i + \varepsilon_i$, avec $\var{\varepsilon_i} = \sigma^2$.
...
\begin{exercice}
\filename{poids_femmes30_39_ans}
\title{Poids moyen des femmes américaines de 30 à 39 ans}
...
\begin{exercice}
\filename{transfos_lineaires_estimateurs}
\title{Effet de transformations linéaires sur les estimateurs}
  ...
  c + d x_i$, $i = 1, \dots, n$.
  \begin{enumerate}
  \item Trouver quel sera l'effet sur les estimateurs des moindres
...
\begin{exercice}
\filename{ajout_point_reg_lin_simple_origine}
\title{Ajout d'un nouveau point}
...
@
    \begin{figure}
<<simple-pointmagique,echo=FALSE>>=
...
\begin{exercice}
\filename{boston_housing}
\title{Marché immobilier dans la région métropolitaine de Boston}
...
\title{Consommation de carburant d'une automobile}
...
@
      ...
      \Sexpr{format(signif(b[2], 4), dec = ",")} x_i +
      \varepsilon_i$, $\varepsilon_i \sim N(0,
      \Sexpr{format(signif(s, 4), dec = ",")}^2)$, où $Y_i$ est la
...
\begin{exercice}
\filename{transfo_heteroscedasticite}
\title{Transformation pour régler un problème d'hétéroscedasticité}
...
\begin{figure}
<<simple-bact3, echo=FALSE>>=
plot(fitted(fit1), rstandard(fit1), pch = 16, ylab = "Studentized Residuals", xlab = "Fitted values")
abline(h = 0)
@
...
\begin{figure}
<<simple-bact4, message=FALSE,echo=FALSE>>=
library(MASS)
boxcox(bact~min, lambda = seq(-0.2, 0.5, len = 20), plotit = TRUE)
boxcox(bact~min, lambda = seq(-2, 2, len = 20), plotit = TRUE)
@
