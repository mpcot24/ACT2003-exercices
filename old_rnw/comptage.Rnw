...
\begin{exercice}
\filename{glm_poisson_r}
\title{Ajustement d'un GLM Poisson en R}
...
\begin{exercice}
\filename{glm_poisson_groupes}
\title{GLM Poisson avec deux groupes}
...
\begin{exercice}
\filename{taux_imperfection}
\title{Taux d'imperfection}
...
\begin{exercice}
\filename{berkeley_admission}
\title{Admission à l'Université Berkeley}
...
\begin{exercice}
\filename{deces_cancer_poumon}
\title{Décès dus au cancer du poumon}
