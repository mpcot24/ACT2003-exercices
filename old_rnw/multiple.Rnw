...
\begin{exercice}
\filename{demo_emc_multiple}
\title{EMC pour la régression linéaire multiple}
...
\begin{exercice}
\filename{matrice_incidence}
\title{Matrice d'incidence et EMC}
...
\begin{exercice}
\filename{cas_particulier_simple}
\title{Cas particulier de la régression linéaire simple}
...
\begin{exercice}
\filename{relations_multiples}
\title{Relations importantes avec la régression linéaire multiple}
...
\begin{exercice}
\filename{pivot_coefficient_estime}
\title{Pivot pour les coefficients estimés}
...
\begin{exercice}
\filename{intervalle_de_confiance}
\title{Intervalles de confiance pour la valeur moyenne et la variable réponse}
...
\begin{exercice}
\filename{test_validite_globale}
\title{Test global de validité du modèle}
...
\begin{exercice}
\filename{estimation_main}
\title{Estimation à la main}
  ...
  \begin{sol}
    ...
    \item Soit $\xvec_0^\top = \begin{bmatrix} 1 & 3,5 & 9 \end{bmatrix}$
      ...
      est
      \begin{align*}
        Y_0 &\in \hat{Y}_0 \pm t_{0,025}(1) s \sqrt{1 + \xvec_0^\top (\Xmat^\top \Xmat)^{-1} \xvec_0} \\
...
\begin{exercice}
\filename{carburant_multiple}
\title{Consommation d'essence avec régression linéaire multiple}
...
\begin{exercice}
\filename{sinistres_auto}
\title{Sinistres annuels moyens par véhicule}
...
\begin{exercice}
\filename{demande_biere}
\title{Modèle de régression pour la demande de bière}
...
\begin{exercice}
\filename{statistique_test}
\title{Statistique pour un test}
...
\begin{exercice}
\filename{anova_trois_variables}
\title{ANOVA avec trois variables}
...
\begin{exercice}
\filename{flotte_taxis}
\title{Flotte de taxis}
...
\begin{exercice}
\filename{postulats}
\title{Vérification des postulats}
...
\begin{exercice}
\filename{modele_tabulaires}
\title{Identification d'un modèle de régression}
...
\begin{exercice}
\filename{regression_ponderee}
\title{Démonstration avec la régression pondérée}
  Considérer le modèle de régression linéaire $\yvec = \Xmat
  \betavec + \epsvec$, où $\Xmat$ est une matrice $n \times (p+1)$,
...
\begin{exercice}
\filename{blue_beta_residus}
\title{BLUE et variance pour $\beta$ selon divers postulats résiduels}
...
\begin{exercice}
\filename{calcul_ponderee}
\title{Calculs de la régression pondérée}
...
\begin{exercice}
\filename{outlier}
\title{Inclusion d'une donnée aberrante}
...
\item Ajustez un modèle linéaire en incluant les 19 points dont le code est 0 et le point de code 1. Quel est l'effet de l'inclusion de ce point sur le $R^2$ et sur les estimations des paramètres? Étudiez le résultat de la fonction \texttt{influence.measures()}.
\item Ajustez un modèle linéaire en incluant les 19 points dont le code est 0 et le point de code 2. Quel est l'effet de l'inclusion de ce point sur le $R^2$ et sur les estimations des paramètres? Étudiez le résultat de la fonction \texttt{influence.measures()}.
\item Ajustez un modèle linéaire en incluant les 19 points dont le code est 0 et le point de code 3. Quel est l'effet de l'inclusion de ce point sur le $R^2$ et sur les estimations des paramètres? Étudiez le résultat de la fonction \texttt{influence.measures()}.
\end{enumerate}
