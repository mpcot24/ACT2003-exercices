...
\begin{exercice}
\filename{contributions_etoiles_nba}
\title{Contribution des joueurs étoiles de la NBA}
...
\begin{exercice}
\filename{groupement_binomiales}
\title{Groupement de données binomiales}
...
\begin{exercice}
\filename{age_kyphosis}
\title{Effet de l'âge sur la kyphosis}
...
\begin{exercice}
\filename{cancer_peau_st_paul}
\title{Cas de cancer de la peau parmi des femmes à St-Paul}
...
\begin{exercice}
\filename{pearson_statistique}
\title{Statistique de Pearson pour le modèle binomial}
