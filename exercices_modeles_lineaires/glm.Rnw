...
f. Pour un détenteur de police entre 17 et 20 ans, avec un véhicule de type D âgé de un à 3 ans, on trouve que $$\widehat{\esp{Y_i}}=\exp\left(\hat{\beta}_0+\hat{\beta}^{MODEL}_D \right)=\exp(5.711739+0.472290)=484.94.$$ On note que cette valeur est beaucoup moins élevée que celle obtenue en f).
