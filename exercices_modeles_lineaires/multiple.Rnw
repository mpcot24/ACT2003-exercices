...
\begin{exercice}
  ...
  \begin{sol}
    ...
    \begin{displaymath}
      \hat\betavec = (\Xmat^\top \Xmat)^{-1} \Xmat^\top \yvec.
    \end{displaymath}
  \end{sol}
\end{exercice}
  ...
  \begin{sol}
    ...
    \item On a
      \begin{align*}
        \Xmat^\top \mat{e}
        &= \Xmat^\top (\mat{y - \hat{y}}) \\
        &= \Xmat^\top (\yvec - \Xmat \hat\betavec) \\
        &= \Xmat^\top \yvec - (\Xmat^\top \Xmat) \hat\betavec \\
      ...
      \begin{align*}
        ...
        \end{bmatrix}.
      \end{align*}
      Par conséquent, $\Xmat^\top \mat{e} = \mat{0}$ se simplifie
     ...
     \item On a
       \begin{align*}
         \hat{\yvec}^\top \mat{e}
         &= (\Xmat\hat\betavec)^\top (\mat{y - \hat{y}}) \\
         &= \hat\betavec^\top \Xmat^\top (\yvec - \Xmat\hat\betavec) \\
         &= \hat\betavec^\top \Xmat^\top \yvec - \hat\betavec^\top
         (\Xmat^\top \Xmat) \hat\betavec \\
         &= \hat\betavec^\top \Xmat^\top \yvec - \hat\betavec^\top
       ...
       $\hat{\yvec}^\top \mat{e} = \hat\betavec^\top \Xmat^\top
       \mat{e}$ et donc, en supposant sans perte de généralité que
       $\hat\betavec \ne \mat{0}$, 
       que $\hat{\yvec}^\top \mat{e} = 0$
       et $\Xmat^\top \mat{e} = \mat{0}$ sont des conditions en
       tous points équivalentes.
     \item On a
       \begin{align*}
         \mat{\hat{y}}^\top \mat{\hat{y}}
         &= (\Xmat \hat\betavec)^\top \Xmat \hat\betavec \\
         &= \hat\betavec^\top (\Xmat^\top \Xmat) \hat\betavec \\
         &= \hat\betavec^\top (\Xmat^\top \Xmat) (\Xmat^\top
         \Xmat)^{-1} \Xmat^\top \yvec \\
         &= \hat\betavec^\top \Xmat^\top \yvec.
       \end{align*}
       ...
       $\mat{\hat{y}}^\top \mat{\hat{y}} = \sum_{t = 1}^n
...
\begin{exercice}
  ...
  \item Démontrer que l'erreur dans la prévision de la valeur moyenne
    ...
    \begin{displaymath}
      ...
      \sigma^2\, \Xmat_0 (\Xmat^\top \Xmat)^{-1} \Xmat_0^\top.
    \end{displaymath}
  ...
  \item Démontrer que l'erreur dans la prévision de $Y_0$ est
    \begin{displaymath}
      \esp{(Y_0 - \hat{Y}_0)^2} =
      \sigma^2\, (1 + \Xmat_0 (\Xmat^\top \Xmat)^{-1} \Xmat_0^\top).
    \end{displaymath}
    Construire un intervalle de confiance de niveau $1 - \alpha$ pour
    $Y_0$.
  \end{enumerate}
    ...
    \item Tout d'abord, $\esp{(\hat{Y}_0 - \esp{Y_0})^2} =
      ...
      contient, ici, qu'une seule valeur. Or, par le théorème de la section~\ref{sec:elements:moments}, la variance est
       \begin{align*}
         ...
         &= \Xmat_0 \varmat{\hat\betavec} \Xmat_0^\top \\
         &= \sigma^2 \Xmat_0 (\Xmat^\top \Xmat)^{-1}
         \Xmat_0^\top.
       \end{align*}
       ...
       \begin{displaymath}
         ...
         \sqrt{\Xmat_0 (\Xmat^\top \Xmat)^{-1} \Xmat_0^\top}.
       \end{displaymath}
     ...
     \item Par le résultat obtenu en a) et en supposant que
       ...
       \begin{align*}
         ...
         &= \sigma^2 (1 + \Xmat_0 (\Xmat^\top \Xmat)^{-1}
         \Xmat_0^\top).
       \end{align*}
       ...
       \begin{displaymath}
         Y_0
         \in \hat{Y}_0 \pm z_{\alpha/2}\, \sigma\,
         \sqrt{1 + \Xmat_0 (\Xmat^\top \Xmat)^{-1} \Xmat_0^\top}.
       \end{displaymath}
...
\begin{exercice}
  ...
  \begin{sol}
    ...
    \item Avec les résultats de la partie a), on a
      \begin{align*}
        \hat\yvec &= \Xmat \hat\betavec =
        \begin{bmatrix}
          17 \\ 12 \\ 13,5 \\ 13,5
        \end{bmatrix}, \\
        \hat\epsvec &= \yvec - \hat\yvec =
        \left[
          \begin{array}{r}
            0 \\ 0 \\ 0,5 \\ -0,5
          \end{array}
    ...
    \item Soit $\Xmat_0 = \begin{bmatrix} 1 & 3,5 & 9 \end{bmatrix}$
      ...
      \begin{align*}
        ...
        &= s^2 (1 + \Xmat_0 (\Xmat^\top \Xmat)^{-1} \Xmat_0^\top) \\
        &= 1,1875.
      \end{align*}
...
\begin{exercice}
  ...
  \begin{align*}
    S(\betavec)
    &= \sum_{t=1}^n w_t (\yvec_t - \Xmat_t^\top \betavec)^2 \\
  ...
  \begin{displaymath}
    (\Xmat^\top\, \mat{W}\, \Xmat) \hat\betavec^* = \Xmat^\top\,
    \mat{W}\,  \yvec,
  \end{displaymath}
  ...
  \begin{sol}
    ...
    $S(\betavec)$ sont $(\Xmat^\top\, \mat{W}\, \Xmat) \hat\betavec^* =
    \Xmat^\top\, \mat{W}\, \yvec$ et l'estimateur des moindres
    carrés pondérés est
    \begin{displaymath}
      \hat\betavec^* = (\Xmat^\top \mat{W} \Xmat)^{-1} \Xmat^\top
      \mat{W} \yvec.
    \end{displaymath}
  \end{sol}
\end{exercice}
...
\begin{figure}[h]
\centering
\centerline{\includegraphics[width=0.98\textwidth]{figure/ResPlots.pdf}}
